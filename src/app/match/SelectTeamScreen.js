...
import { createTeam, fetchMyTeams } from "../../services/teamService";
...
const SelectTeamScreen = ({ navigation, route }) => {
  ...
  const loadTeamsFromApi = useCallback(async () => {
    try {
      ...
      const teamsData = res?.data?.teams || res?.data || res?.teams || [];
      if (Array.isArray(teamsData)) {
        // Normalize backend shape to local shape used by UI
        const normalized = teamsData.map((t) => ({
          id: t._id || t.id || Date.now().toString(),
          teamName: t.name || t.teamName || "Unnamed Team",
          avatarColor: t.avatarColor || AVATAR_COLORS[Math.floor(Math.random() * AVATAR_COLORS.length)],
          avatarInitials: t.avatarInitials ||
            (t.name ? (t.name.split(" ").length >= 2
              ? (t.name.split(" ")[0][0] + t.name.split(" ")[1][0]).toUpperCase()
              : t.name.substring(0, 2).toUpperCase()) : "TM"),
      ...
      console.warn("[LOAD TEAMS ERROR]:", err?.message || err);
      // Silently fail — empty list will show "no teams" state
    } finally {
  ...
  useEffect(() => {
    ...
    loadTeamsFromApi();
  }, [route.params?.teamType, loadTeamsFromApi]);
  ...
  // Handle New Team Creation -> Calls POST /api/v1/teams -> Saves to DB
  const handleCreateTeam = async () => {
    ...
    try {
      ...
      // ✅ POST /api/v1/teams — Backend DB me permanent save
      const res = await createTeam(payload);
      const savedTeam = res?.data?.team || res?.data || res?.team || res;

      // Normalize backend response to local UI shape
      const newTeamForUi = {
        id: savedTeam?._id || savedTeam?.id || Date.now().toString(),
        teamName: savedTeam?.name || teamName.trim(),
        avatarColor: randomColor,
        avatarInitials: initials,
        location: savedTeam?.location || city.trim() || "India",
        captainName: captainName.trim() || "Captain",
        captainPhone: captainPhone.trim() || "",
        logoUrl: savedTeam?.logoUrl || null,
        description: savedTeam?.description || "",
        isOpponent: teamType === "B",
      };

      // Update local list with backend-confirmed team
      setTeams((prev) => [newTeamForUi, ...prev]);
      ...
      // Navigate to TeamRoster screen with real DB-saved team
      ...
      Alert.alert(
        ...
        err?.message || "Could not save team. Please check your connection and try again."
      );
    } finally {
          ...
          <View className="flex-1 bg-white">
            ...
            {isLoadingTeams ? (
              <View className="flex-1 items-center justify-center pt-20">
                ...
                <Text className="text-slate-400 text-sm font-medium mt-3">Loading your teams...</Text>
              </View>
            ) : (
              <FlatList
                data={filteredTeams}
                ...
                ListEmptyComponent={
                  <View className="pt-16 items-center justify-center">
                    <Text className="text-slate-400 text-sm font-medium mb-3">
                      No {activeTab === "Opponents" ? "opponent" : ""} teams saved yet.
                    </Text>
                    <TouchableOpacity
